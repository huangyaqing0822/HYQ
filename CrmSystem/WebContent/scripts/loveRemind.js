$(function(){	
	...
	$("#loveRemind_add").dialog({
		...
		buttons:[{
			...
			iconCls:"icon-add",
			handler:function(){
				if($("#loveRemind_add").form("validate")){
					$.ajax({
						url:"../clientManagementAction/addLovingPost",
						type:"post",
						data:{
							"clWarnname":$("input[name='clWarnname']").val(),
							"clWarntime":$("input[name='clWarntime']").val(),
							"clWarntype":$("input[name='clWarntype']").val(),
							"clPeriod":$("input[name='clPeriod']").val(),
							"clRate":$("#clRate").val(),
							"clRemarkremind":$("#clRemarkremind").val(),					
						},
						success : function(result) {
							...
							$("#loveRemind_add").dialog("close").form("reset");
							$("#loveRemind").datagrid("reload");  
						}
					});
				} 
			},
	...
	//修改提交
	 $("#loveRemind_edit").dialog({
			...
			buttons:[{
				...
				iconCls:"icon-edit",
				handler:function(){
					if($("#loveRemind_edit").form("validate")){
						var rows = $('#loveRemind').datagrid('getSelections');
						$.ajax({						
							...
							success : function(result) {	
						    	if(result == "true"){
						    		...
						    		$('#loveRemind_edit').dialog('close');
									$('#loveRemind').datagrid('reload');  //到首页刷新
						    	}else{
				...
				iconCls:"icon-redo",
				handler:function(){
					$("#loveRemind_edit").dialog("close").form("reset");
				},
			}], 
		});
